...
function getTools(){
	 return request.get('/admin/api/v1/deepseek/getTools');
}
...
// Agent模式流式请求方法（支持工具调用）
async function searchAgentStream(data, onMessage, onError, onComplete) {
  const baseURL = import.meta.env.VITE_APP_BASE_API || '';
  const url = `${baseURL}/admin/api/v1/deepseek/search/agent/stream`;
  ...
  try {
    ...
    console.error('Agent流式请求错误:', error);
    if (onError) onError(error);
  }
}

export default{
	 search, getSession, deleteSession, getKey, searchStream, searchAgentStream, getTools
}