  ...
  <div>
    ...
    <el-space :size="24" style="margin-bottom: 16px;">
      ...
      <div class="summary-item">
        <div class="summary-label">结算销量合计</div>
        <div class="summary-value" style="color: #FF6700;">{{ settlementTotal }}</div>
      </div>
      <div class="summary-item">
        <div class="summary-label">订单销量合计</div>
...
function loadData() {
  ...
  settlementApi.quantityByDay(params).then(res => {
    ...
    renderQuantityChart(labels, datas, orderDatas);
    // 如果每日收入数据还没返回，先用旧数据渲染费用图表
  ...
  // 请求每日收入明细数据（费用图表 + 表格）
  settlementApi.dailyIncomeReport(params).then(res => {
...
// 渲染销量图表
function renderQuantityChart(labels, datas, orderDatas) {
