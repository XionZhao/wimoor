...
public class FeiShuTemplateStrategy implements IFinClosingTemplateStrategy {
    ...
    public void generateVoucher(UserInfo userInfo, String templateId, String periodCode) {
        ...
        for(int i=0;i<records.size();i++) {
            ...
            BigDecimal amount = extractBigDecimalValue(recordItemFields, finClosingTemplateFeishu.getAmountField());
    ...
    // JSONObject 类型，单选字段等可能返回对象格式 { "text": "xxx", "value": "xxx" }
    if (fieldValue instanceof JSONObject) {
        JSONObject jsonObj = (JSONObject) fieldValue;
    ...
    /**
     * 从飞书记录中安全提取 BigDecimal 金额值。
     * 飞书表格的金额字段可能返回 JSONArray（如货币字段），需要特殊处理。
     */
    private BigDecimal extractBigDecimalValue(JSONObject recordItemFields, String fieldName) {
        if (recordItemFields == null || StrUtil.isBlank(fieldName)) {
            return null;
        }
        if (!recordItemFields.containsKey(fieldName)) {
        ...
        if (fieldValue instanceof JSONArray) {
            ...
            if (firstItem instanceof Number) {
                return new BigDecimal(firstItem.toString());
            }
            if (firstItem instanceof JSONObject) {
                JSONObject jsonItem = (JSONObject) firstItem;
                // 飞书货币字段可能以 { "text": "100.00", "value": 100 } 结构返回
                if (jsonItem.containsKey("value")) {
                    Object val = jsonItem.get("value");
            ...
            if (firstItem instanceof String) {
                try {
                    return new BigDecimal(((String) firstItem).replaceAll("[,，\\s]", ""));
                } catch (NumberFormatException e) {
                    // ignore
                }
            }
