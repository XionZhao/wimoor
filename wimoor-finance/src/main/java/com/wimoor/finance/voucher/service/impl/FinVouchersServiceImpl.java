...
public class FinVouchersServiceImpl implements IFinVouchersService
{
    ...
    public List<FinVouchers> selectFinVouchersList(FinVoucherDTO finVouchers)
    {
        ...
        for (FinVouchers v : list) {
            ...
            // 7. 标记会计期间是否已结账
            v.setPeriodClosed(isPeriodClosed(v.getGroupid(), v.getVoucherDate()));
        }
    ...
    public int insertFinVouchers(FinVouchers finVouchers)
    {
        ...
        // 如果有附件，设置附件数量
        if (finVouchers.getFiles() != null && !finVouchers.getFiles().isEmpty()) {
            finVouchers.setAttachmentCount(finVouchers.getFiles().size());
        }
        // 1. 保存凭证主表
...
//             }
    ...
    private BalanceResult calculateEndBalance(BigDecimal beginBalance, Integer beginDirection,
        ...
        // 统一存储为绝对值 + 方向，与 AccountingClosingService 保持一致
        if (endBalance.compareTo(BigDecimal.ZERO) == 0) {
            endDirection = subject.isDebitBalance() ? 1 : 2;
        } else if (endBalance.compareTo(BigDecimal.ZERO) > 0) {
            endDirection = subject.isDebitBalance() ? 1 : 2;
        } else {
            endDirection = subject.isDebitBalance() ? 2 : 1;
    ...
    public int filesFinVouchers(FinVouchers finVouchers) {
        ...
        // 同步更新附件数量到 fin_vouchers 表
        int fileCount = finVouchers.getFiles() != null ? finVouchers.getFiles().size() : 0;
        FinVouchers updateParam = new FinVouchers();
        updateParam.setVoucherId(finVouchers.getVoucherId());
        updateParam.setAttachmentCount(fileCount);
        finVouchersMapper.updateFinVouchers(updateParam);
        return count;
    }
