...
public class FinAccountingSubjectsServiceImpl implements IFinAccountingSubjectsService
{
    ...
    @Override
    public List<FinAccountingSubjects> selectByParentCode(String groupid, String parentCode) {
        FinAccountingSubjects query = new FinAccountingSubjects();
        query.setGroupid(groupid);
        query.setParentCode(parentCode);
        query.setStatus(1);
        return finAccountingSubjectsMapper.selectFinAccountingSubjectsList(query);
    }
    ...
    public void setFinLedgerSubject(TableDataInfo page) {
        if(page != null && !page.getRows().isEmpty()){
            ...
            for (Object row : page.getRows()) {
                    ...
                    // 跳过汇总行（期初余额、本期合计、本年累计），保留已设置的subjectName
                    Object summary = voucher.get("summary");
                    if (summary != null && ("期初余额".equals(summary.toString()) || 
                        "本期合计".equals(summary.toString()) || 
                        "本年累计".equals(summary.toString()))) {
                        continue;
                    }
    ...
    public List<FinAccountingSubjects> getProfitLossSubjects(String groupid) {
        // 费用类科目编码前缀：5
        // 如：5601销售费用、5602管理费用等
        List<String> profitLossPrefixes = Arrays.asList("5");
    ...
    /**
     * 获取利润分配科目
     * 利润分配科目编码为 3104（小企业会计准则）
     */
    @Override
    public FinAccountingSubjects getProfitDistributionSubject(String groupid) {
        // 利润分配科目：3104
        return finAccountingSubjectsMapper.selectBySubjectCode(groupid, "3104");
    }
     ...
     * @param groupId 租户ID
     * @param importType 导入类型：increment-增量导入，override-覆盖导入
     * @return 导入成功的记录数
     * @throws IOException 异常
     */
    @Override
    public int importSubjects(InputStream inputStream, String groupId, String importType) throws IOException {
        ...
        int count = 0;
        // 收集Excel中所有科目编码，用于覆盖导入时禁用不在Excel中的科目
        List<String> excelSubjectCodes = new ArrayList<>();
        ...
        for (FinAccountingSubjects subject : list) {
            ...
            if (subject.getSubjectCode() != null) {
                // 移除所有空格，确保编码格式正确
                String subjectCode = subject.getSubjectCode().trim().replaceAll("\\s+", "");
                ...
                // 收集科目编码
                excelSubjectCodes.add(subjectCode);
            }
            subject.setUpdatedTime(new Date());
            // 状态以Excel中"状态"列的值为准，"启用"=1，"停用"=0
            
            if ("override".equals(importType)) {
                // 覆盖导入：不限状态查询，找到已存在的记录（包括已停用的）
                FinAccountingSubjects existing = finAccountingSubjectsMapper.selectByGroupCodeAnyStatus(subject.getGroupid(), subject.getSubjectCode());
                if (existing == null) {
                    // 新增科目
        ...
        // 覆盖导入：将不在Excel中的启用科目设为停用
        if ("override".equals(importType)) {
            finAccountingSubjectsMapper.batchDisableSubjects(groupId, excelSubjectCodes);
        }
     ...
     */
    @Override
    public String getSubjectCodeById(String groupid, String subjectId) {
        if (subjectId == null || subjectId.isEmpty()) {
            return null;
        }
        FinAccountingSubjects subject = selectFinAccountingSubjectsBySubjectId(subjectId);
        if (subject != null) {
