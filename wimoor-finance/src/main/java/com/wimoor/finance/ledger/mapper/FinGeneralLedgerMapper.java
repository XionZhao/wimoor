...
public interface FinGeneralLedgerMapper
{
    ...
    /**
     * 查询某科目在指定期间之前最近的记录（前推查找）
     */
    Map<String, Object> getLatestBalanceBeforePeriod(@Param("groupid") String groupid,
                                                      @Param("subjectCode") String subjectCode,
                                                      @Param("period") String period);

    /**
     * 查询所有科目在指定期间之前最近的记录（带前推查找）
     */
    List<Map<String, Object>> getAllSubjectBalanceWithFallback(@Param("groupid") String groupid,
                                                                @Param("period") String period);

    /**
     * 计算本年累计余额：从1月累加到指定期间的借方和贷方发生额
     * 用于利润表的本年累计金额计算
     *
     * @param groupid 租户ID
     * @param period 期间（格式：YYYYMM）
     * @return 各科目的本年累计借方和贷方发生额
     */
    List<Map<String, Object>> getAllSubjectYearToDateBalance(@Param("groupid") String groupid,
                                                              @Param("period") String period);
    ...
    /**
     * 从凭证分录汇总本年累计借贷发生额（从1月1日到指定期间月末）
     * 直接从原始凭证计算，不受结账状态影响
     *
     * @param groupid 租户ID
     * @param period 期间（格式：YYYYMM）
     * @return 各科目的本年累计借方和贷方发生额及年初余额
     */
    List<Map<String, Object>> sumYearToDateEntriesBySubject(@Param("groupid") String groupid,
                                                            @Param("period") String period);
}
