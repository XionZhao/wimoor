...
public class FinGeneralLedgerServiceImpl implements IFinGeneralLedgerService 
{
    ...
    @Override
    public Map<String, Object> getLatestBalanceBeforePeriod(String groupid, String subjectCode, String period) {
        return this.finGeneralLedgerMapper.getLatestBalanceBeforePeriod(groupid, subjectCode, period);
    }

    @Override
    public List<Map<String, Object>> getAllSubjectBalanceWithFallback(String groupid, String period) {
        return this.finGeneralLedgerMapper.getAllSubjectBalanceWithFallback(groupid, period);
    }
    ...
    public List<Map<String, Object>> selectTopLevelSummaryFull(FinLedgerDTO dto) {
        ...
        // 4.1 查询上个月的期末余额作为期初余额（解决单独查询某月时期初余额为0的问题）
        Map<String, BigDecimal[]> prevMonthEndBalanceMap = new LinkedHashMap<>();
        String prevMonth = getPrevPeriod(dto.getStartPeriod());
        List<Map<String, Object>> prevMonthData = this.finGeneralLedgerMapper.selectTopLevelSummaryByDateRange(
            dto.getGroupid(), prevMonth, prevMonth);
        if (prevMonthData != null) {
            for (Map<String, Object> item : prevMonthData) {
                String sc = item.get("subjectCode").toString();
                BigDecimal endBalance = item.get("endBalance") != null ? new BigDecimal(item.get("endBalance").toString()) : BigDecimal.ZERO;
                Integer endDirection = item.get("endDirection") != null ? Integer.parseInt(item.get("endDirection").toString()) : 1;
                prevMonthEndBalanceMap.put(sc, new BigDecimal[]{endBalance, new BigDecimal(endDirection)});
            }
        }
        ...
        for (Map.Entry<String, List<Map<String, Object>>> entry : subjectMap.entrySet()) {
            ...
            // 初始化期初余额：使用上个月的期末余额（2月的期初 = 1月的期末）
            BigDecimal[] prevMonthBalance = prevMonthEndBalanceMap.get(subjectCode);
            if (prevMonthBalance != null) {
                lastEndBalance = prevMonthBalance[0];
                lastEndDirection = prevMonthBalance[1].intValue();
            }
            ...
            for (String period : allPeriods) {
                ...
                // 期初余额只在第一个期间显示
                if (allPeriods.indexOf(period) == 0) {
    ...
    public int rebuildGeneralLedger(String groupid) {
        ...
        // 4. 逐科目、逐期间重建总账（与常规过账统一：存绝对值 + endDirection 区分方向）
        int count = 0;
        for (Map.Entry<String, List<Map<String, Object>>> subjectEntry : subjectMap.entrySet()) {
            ...
            // 获取科目类型，确定余额方向
            FinAccountingSubjects subject = finAccountingSubjectsService.getSubjectById(subjectId);
            boolean isDebitBalance = subject != null && subject.isDebitBalance();
            ...
            for (Map<String, Object> periodData : periodList) {
                ...
                if (isDebitBalance) {
                    ...
                    // 贷方余额科目（负债、权益、收入类）：贷增借减
                    signedBalance = signedBalance.add(creditTotal).subtract(debitTotal);
                }

                // 统一存储为绝对值 + 方向（与常规过账 calculateEndBalance 一致）
                Integer endDirection;
                BigDecimal endBalance;
                if (signedBalance.compareTo(BigDecimal.ZERO) >= 0) {
                    endDirection = isDebitBalance ? 1 : 2;
                    endBalance = signedBalance;
                } else {
                    endDirection = isDebitBalance ? 2 : 1;
                    endBalance = signedBalance.abs();
                }
                ...
                ledger.setBeginBalance(BigDecimal.ZERO); // 将在 fillBeginBalances 中回填
                ledger.setBeginDirection(isDebitBalance ? 1 : 2);
    ...
    private void fillBeginBalances(String groupid) {
        ...
        for (List<FinGeneralLedger> ledgerList : subjectMap.values()) {
            ...
            // 获取科目类型
            FinAccountingSubjects subject = finAccountingSubjectsService.getSubjectById(
                    String.valueOf(ledgerList.get(0).getSubjectId()));
            boolean isDebitBalance = subject != null && subject.isDebitBalance();
            ...
            for (int i = 0; i < ledgerList.size(); i++) {
                ...
                if (i == 0) {
                    ...
                    String firstPeriod = current.getPeriod();
                    String prevPeriod = getPrevPeriod(firstPeriod);
                    ...
                    if (prevLedger != null) {
                        ...
                        current.setBeginDirection(isDebitBalance ? 1 : 2);
                    }
                } else {
                ...
                // 重新计算期末余额（与常规过账 calculateEndBalance 一致）
                ...
                // 将 beginBalance 转换为带符号值：方向与科目自然方向一致则为正，否则为负
                boolean isNaturalDirection = (isDebitBalance && beginDirection == 1) 
                        || (!isDebitBalance && beginDirection == 2);
                BigDecimal signedBegin = isNaturalDirection ? beginBalance : beginBalance.negate();
                
                // 根据科目类型计算
                BigDecimal signedEnd;
                if (isDebitBalance) {
                    signedEnd = signedBegin.add(debitTotal).subtract(creditTotal);
                } else {
                    signedEnd = signedBegin.add(creditTotal).subtract(debitTotal);
                }
                
                // 统一存储为绝对值 + 方向
                Integer endDirection;
                BigDecimal endBalance;
                if (signedEnd.compareTo(BigDecimal.ZERO) >= 0) {
                    endDirection = isDebitBalance ? 1 : 2;
                    endBalance = signedEnd;
                } else {
                    endDirection = isDebitBalance ? 2 : 1;
                    endBalance = signedEnd.abs();
                }
